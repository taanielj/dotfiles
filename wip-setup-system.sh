...
log() {
    if [[ "$1" == "-n"]]; then # for doing cool stuff like "Installing... Done" or whatever, prints without newline
        shift
        echo -ne "${BLUE}$*${RESET}"
    else    
        echo -e "${BLUE}$*${RESET}"
...
install_packages() {
    ...
    if [[ ${#not_installed[@]} -gt 0 ]]; then
        if [[ "$os" == "Linux" ]]; then
            sudo apt install -y -qq ${not_installed[@]} 
        elif [[ "$os" == "Darwin" ]]; then
            grep_list="is already installed|To reinstall|brew reinstall|the latest version is already installed"
            brew install "${not_installed[@]}" 2>&1 | grep -vE "$grep_list"
        fi
    fi    
}
# since no good way to install without sudo
install_lazygit
